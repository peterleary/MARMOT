...
fp <- "~/Desktop/IMCR/MARMOT/For_Submission/MARMOT_Paper/"
...
parcScriptDir <- "~/Desktop/FGCZ/MARMOT/"
...
RDataFolder <- "~/Desktop/IMCR/MARMOT/For_Submission/MARMOT_Paper/Results_Files_2025-02-28_18.13.46/"
...
heightToPlot <- ceiling(nRowsToPlot*4)
widthToPlot <- ceiling((4*1.1)*nColsToPlot)
```
...
clusterPvalues <- lapply(levels(kids), function(k) {
  ...
  if(is.na(pvals)) {pvals <- 1}
  y_max <- max(clusterBoxPlotsDFMelt$value)
  spacing <- (y_max - min(clusterBoxPlotsDFMelt$value)) * 0.1
...
```{r, label = "draw marker boxplots", fig.height = heightToPlot, fig.width = widthToPlot}
