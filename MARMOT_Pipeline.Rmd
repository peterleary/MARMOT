---
title: "MARMOT Flow Cytometry Pipeline v1.0.0"
author: ''
...
```{r, label = "input settings", cache = FALSE}
...
RDataFolder <- NULL
# RDataFolder <- "~/MARMOT_Paper/Results_Files_2024-10-28_15.41.21/"
...
requiredVariables <- c(
  ...
  "nCores", "themeToUse", "viridisColour", "cache"
)
...
knitr::opts_chunk$set(
  comment = '', fig.width = 8, fig.height = 8, warning = FALSE, error = FALSE, message = FALSE, cache = cache, echo = FALSE, cache.lazy = FALSE, dev = 'png'
)
timeRun <- format(Sys.time(), "%Y-%m-%d_%H.%M.%S")
options(bitmapType='cairo')
```

```{r, label = "load packages", include = FALSE, cache = FALSE}
library(flowCore)
library(FlowSOM)
library(tidyverse)
library(CATALYST)
...
if(!is.null(RDataFolder)) {
  ...
  for (i in seq_along(fileNames)) {
    assign(varNames[i], qread(fileNames[i], nthreads = 8), envir = .GlobalEnv)
  }
...
```{r, label = "import md file", results = "hide", cache = F}
# md = sample metadata
md <- read_xlsx(file.path(fp, list.files(fp, pattern = "(?i)metadata|(?i)meta") %>% .[grep("^\\~\\$", ., invert = T)]), sheet = 2)

# smd <- study meatadata
smd <- read_xlsx(file.path(fp, list.files(fp, pattern = "(?i)metadata|(?i)meta") %>% .[grep("^\\~\\$", ., invert = T)]), sheet = 1)
...
```{r, label = "Cluster colours", results = "hide", cache = F}
...
```{r, label = "import fcs files", results = "asis", include = FALSE, dependson = c("input settings", "import md file"), eval = is.null(RDataFolder), cache = F}
...
```{r, label = "arrange frames list", results = "asis", include = FALSE, dependson = -2, eval = is.null(RDataFolder), cache = F}
...
```{r, label = "make cofactor histograms", results = "hide", dependson = "import fcs files and qc", eval = is.null(RDataFolder)}
...
cofactorPlotList <- setNames(lapply(useMarkers, function(marker) {
  # Make the plots
  markersToPlot <- setNames(mclapply(c("pretransform", paste0("Cofactor_", cofactorsToPlot)), function(cofactors) {
  ...
  }, mc.cores = 1), c("pretransform", paste0("Cofactor_",cofactorsToPlot)))
...
```{r, label = "make sce", results = "hide", dependson = c("import md file", "arrange frames list"), eval = is.null(RDataFolder), cache = F}
...
```{r, label = "get some variables", results = "hide", cache = F}
...
if (grepl("Rphenograph|FastPG", clusteringMethodToUse)) {
  ...
  if (clusteringMethodToUse == "FastPG") {
    kList <- setNames(mclapply(kValuesIWant, function(i) {
      set.seed(2)
    ...
    }, mc.cores = nCores), paste0("k", kValuesIWant))
  }
...
```{r, label = "parc", results = "hide", dependson = "input settings", cache = FALSE, eval = is.null(RDataFolder)}
# PARC-based
...
```{r, label = "FlowSOM", results = "hide", dependson = "input settings", cache = FALSE, eval = is.null(RDataFolder)}
...
```{r, label = "Dim Red", results = "hide", dependson = "input settings", cache = FALSE, eval = is.null(RDataFolder)}
...
```{r, label = "DR all cells", fig.height=6, fig.width=(3.5*ceiling(length(levels(umapDFList$All$cluster_id))/18)+0.5), results = 'asis'}
if (!is.null(drCellPerCondition)) {
  cat("##### All Cells", dimRedMethodToUse, "\n\n")
  cat("Since you clustered on all available cells (after initial downsampling), here is a DR plot of all available cells. Your per-condition DR plots are subset from this main DR plot.\n\n")
...
```{r, label = "main cluster DR 1", fig.height=6, fig.width=(3.5*ceiling(length(levels(umapDFList$All$cluster_id))/18))+0.5}
if (is.null(drCellPerCondition)) {
  umapsDFToPlot <- "All"
} else {
...
if (gimmePDFs) {
  ...
  lapply(names(UMAPList), function(m) {
    ...
    dev.off()
    mclapply(names(UMAPList[[m]]), function(n) {
...
DRperCondList <- setNames(lapply(c("With", "Without"), function(yn) {
  ...
  l1 <- setNames(mclapply(names(useMarkers), function(marker) {
    m1 <- plotUMAPNice(df = umapDFList[[umapsDFToPlot]], colour_by = marker, name = "Normalised\nExpression\n", size = 0.3, show_cluster_labels = var) + 
      facet_wrap(~condition, ncol = 2)
    m1 <- m1 + theme(plot.title = element_blank(), axis.title = element_blank(), legend.position = "hide")
    m1
  }, mc.cores = nCores), useMarkers)
  return(l1)
}), c("With Labels", "Without Labels"))
```

```{r, label = "DR per per marker per cond 2", fig.width=10, fig.height=5*(ceiling(length(condLevels)/2)), results = 'asis'}
...
if (gimmePDFs) {
  ...
  lapply(names(DRperCondList), function(m) {
    ...
    mclapply(names(DRperCondList[[m]]), function(n) {
...
```{r, label = "cluster boxplot", fig.width=15, fig.height=(nlevels(sce$cluster_id)/8)*3}
clusterBoxPlotsDF <- pFH@matrix
...
clusterBoxplot <- ggplot(clusterBoxPlotsDFMelt, aes(x = condition, y = value)) + 
  geom_boxplot(aes(fill = condition), alpha = 0.2, show.legend = F, outlier.shape = NA) +
  geom_beeswarm(aes(fill = condition), pch = 21, size = 3, stroke = 0.3, cex = 3, corral = "random", corral.width = 0.9) +
  facet_wrap(~variable, ncol = 8, scales = "free") +
  ylab("Normalised Frequency") + xlab("Condition") +
  geom_vline(xintercept = (seq_along(condLevels)+0.5)[-length(condLevels)], linetype = "dashed", alpha = 0.6) +
  scale_fill_manual(values = coloursList$condition) +
  theme_prism(axis_text_angle = 45)
clusterBoxplot
openxlsx::write.xlsx(as.data.frame(clusterBoxPlotsDF), file = file.path(excelDir, "Cluster_per_sample_normalised_abundance.xlsx"))
```

```{r, label = "save cluster boxplot", include = F}
if (gimmePDFs) {
  pdf(file = file.path(pdfDir, "Cluster_Boxplot.pdf"), width = 15, height = (nlevels(sce$cluster_id)/8)*3)
...
```{r, label = "marker boxplots by condition 1", fig.width=18, fig.height=ceiling(length(useMarkers)/8)*3}
medianScaled <- CATALYST:::.agg(sce, "sample_id", "median", "exprsQuantNorm")
markerBoxplotsDF <- melt(medianScaled, varnames = c("antigen", "sample_id"[length("sample_id")]))
i <- match(markerBoxplotsDF$sample_id, sce$sample_id)
j <- setdiff(names(colData(sce)), c(names(markerBoxplotsDF), "cluster_id"))
markerBoxplotsDF <- cbind(markerBoxplotsDF, colData(sce)[i, j, drop = FALSE])
openxlsx::write.xlsx(as.data.frame(markerBoxplotsDF), file = file.path(excelDir, "Median_Markers_Per_Sample_Expression.xlsx"))
markersBoxplot <- ggplot(markerBoxplotsDF, aes(x = condition, y = value)) +
  geom_boxplot(aes(fill = condition), alpha = 0.2, show.legend = F, outlier.shape = NA) +
  geom_beeswarm(aes(fill = condition), pch = 21, size = 3, stroke = 0.3, cex = 3, corral = "random", corral.width = 0.9) +
  ...
  scale_fill_manual(values = coloursList$condition) +
  theme_prism(axis_text_angle = 45) +
  ylab("Median Quantile Normalised Expression") + xlab("Condition")
markersBoxplot
```

```{r, label = "save marker boxplot 1", include = F}
if (gimmePDFs) {
  pdf(file = file.path(pdfDir, "Marker_per_Sample_boxplot.pdf"), width = 18, height = ceiling(length(useMarkers)/8)*3)
  print(markersBoxplot)
  dev.off()
}
```
...
```{r, label = "markers per cluster boxplot", fig.width=12, fig.height=5, results = 'asis'}
medianScaled <- CATALYST:::.agg(sce, "sample_id", "median", "exprsQuantNorm")
markerBoxplotsDF <- melt(medianScaled, varnames = c("antigen", c("cluster_id", "sample_id")[length(c("cluster_id", "sample_id"))]))
i <- match(markerBoxplotsDF$sample_id, sce$sample_id)
j <- setdiff(names(colData(sce)), c(names(markerBoxplotsDF), "cluster_id"))
markerBoxplotsDF <- cbind(markerBoxplotsDF, colData(sce)[i, j, drop = FALSE])
...
individualClusterMarkerBoxplots <- setNames(lapply(levels(markerBoxplotsDF[[paste0(mergeBy, knn)]]), function(i) {
  cat("##### Cluster", i, "\n\n")
  ...
  cat("\n\n")
  return("g1" = g1)
}), levels(markerBoxplotsDF[[paste0(mergeBy, knn)]]))
```
...
```{r, label = "markers_boxplots2", fig.width=10, fig.height=6, results = "asis"}
for (i in seq_along(selectedClustersList)) {
