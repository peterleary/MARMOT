...
# Cross-platform parallel processing setup
# Detect available cores safely
max_cores <- parallel::detectCores(logical = FALSE)
nCores <- min(nCores, max_cores - 1)  # Leave one core free

# Platform-specific optimisations
if (.Platform$OS.type == "windows") {
  # Windows: Force multisession, disable forking
  options(future.fork.enable = FALSE)
  plan(multisession, workers = nCores)
} else if (Sys.info()["sysname"] == "Darwin") {
  # macOS: Check if running under Rosetta or native
  arch_info <- system("uname -m", intern = TRUE)
  r_arch <- R.version$arch
  
  if (r_arch == "x86_64" && arch_info == "arm64") {
    # Running x86_64 R on Apple Silicon (Rosetta)
    options(future.fork.enable = FALSE)
    plan(multisession, workers = min(nCores, 4))  # Limit workers under Rosetta
  } else {
    # Native macOS (Intel or ARM)
...
```{r cluster_colours, results = "hide", cache = TRUE}
...
```{r create_panel, results = "hide", eval = is.null(RDataFolder), cache = TRUE}
...
```{r run_flow_auto_qc, results = "asis", include = FALSE, dependson = -1, eval = is.null(RDataFolder), cache = TRUE}
...
if (clusteringMethodToUse %in% c("Rphenograph", "FastPG")) {
  cat("\n🧠 Running clustering using", clusteringMethodToUse, "...\n\n")
...
```{r make_seurat_object, include = F, eval = is.null(RDataFolder)}
