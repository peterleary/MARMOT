  ...
  markerPairsList <- setNames(lapply(markerPairTypes, function(x) {
    ...
    plots <- lapply(toPlot, function(y) {
      ...
      if (!quantileNormaliseAll) {
        ...
        q1 <- ggplot(es, aes(x = .data[[markersToPlot[1]]], y = .data[[markersToPlot[2]]], color = .data[[mergeBy]])) + geom_point(size = 0.001) +
          geom_label_repel_quiet(data = median, aes(label = .data[[mergeBy]], x = x, y = y), show.legend = FALSE, size = 2) +
      ...
      colnames(median)[1] <- mergeBy
      p1 <- ggplot(esQN, aes(x = .data[[markersToPlot[1]]], y = .data[[markersToPlot[2]]], color = .data[[mergeBy]])) + geom_point(size = 0.001) +
        geom_label_repel_quiet(data = median, aes(label = .data[[mergeBy]], x = x, y = y), show.legend = FALSE, size = 2) +
        scale_color_manual(values = coloursList[[paste0(mergeBy, knn)]]) + 
        theme_prism() +
        theme(legend.position = "hide") +
...
DRSelectedClusters <- setNames(lapply(seq_along(clustersToMapList), function(i) {
  ...
  gp <- gp + geom_text_repel_quiet(data = cluster_centre, aes(x = center_x, y = center_y, label = cluster_id, colour = cluster_id), size = 4)
...
DRSelectedClusters2 <- setNames(lapply(seq_along(clustersToMapList), function(i) {
  ...
  gp <- gp + geom_text_repel_quiet(data = cluster_centre, aes(x = center_x, y = center_y, label = cluster_id, colour = cluster_id), size = 4, show.legend = F)
