    ...
    parcClustList <- setNames(future_lapply(kValuesIWant, function(k) {
      tryCatch(
        run_parc(
          mat = esPARC,
          k = k,
          num_threads = max(1L, floor(nCores / length(kValuesIWant)))
        ),
        error = function(e) NULL
      )
    }, future.seed = TRUE), paste0("p", kValuesIWant))
  
    if (all(vapply(parcClustList, is.null, logical(1)))) {
      message("⚠️ PARC unavailable → switching clusteringMethodToUse = 'FlowSOM'")
      clusteringMethodToUse <- "FlowSOM"
    } else {
      for (k in kValuesIWant) {
        if (!is.null(parcClustList[[paste0("p", k)]])) {
          cluster_labels <- factor(parcClustList[[paste0("p", k)]] + 1)
          # Add "p" prefix and apply mixed sort
      ...
      metadata(sce)$cluster_codes <- setNames(
        list(factor(levels(sce[[chosen_k]]))),
...
if (is.null(RDataFolder)) {
  ...
  if (dimRedMethodToUse == "pacmap") {
    es <- t(assay(sce, exprsToUse)[featuresToUmap, ])
    embedding <- tryCatch(
      run_pacmap(es, verbose = TRUE, apply_pca = FALSE),
      error = function(e) NULL
    )
  ...
  } else if (dimRedMethodToUse == "TSNE") {
