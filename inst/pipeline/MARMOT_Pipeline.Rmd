...
if (is.null(RDataFolder)) {
  if (clusteringMethodToUse == "PARC") {
    ...
    parcClustList <- setNames(future_lapply(kValuesIWant, function(k) {
      parc_clust(esPARC, knn = as.integer(k), num_threads = max(1L, floor(nCores / length(kValuesIWant))))
...
if (is.null(RDataFolder)) {
  ...
  if (dimRedMethodToUse == "pacmap") {
    ...
    reducedDim(sce, dimRedMethodToUse) <- pacmap_fit(es, verbose = TRUE, apply_pca = FALSE)
  }
