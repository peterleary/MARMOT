...
requiredVariables <- c(
  ...
  "drCellCount", "runQC", "metricsQC", "useQC", "RDataFolder", "excludeTheseSamples",
...
tryCatch({
  # First check if the class exists at all
  if (!exists("ExpData", mode="S4")) {
    # Create the class union in the global environment
    methods::setClassUnion("ExpData", c("matrix", "SummarizedExperiment"), 
                 where = .GlobalEnv)
  }
}, error = function(e) {
  message("Note: Attempted to create ExpData class union but encountered: ", e$message)
})
```
...
# md = sample metadata
md <- read_xlsx(file.path(fp, md_fp), sheet = "File Data")
# smd <- study meatadata
smd <- read_xlsx(file.path(fp, md_fp), sheet = "Study Data")
...
cofactorPlotList <- setNames(future_lapply(useMarkers, function(marker) {
  ...
  markersToPlot <- setNames(future_lapply(c("pretransform", paste0("Cofactor_", cofactorsToPlot)), function(cofactors) {
    ggplot(cofactorDFList[[marker]][["df"]], aes(x = .data[[cofactors]], y = .data[[..ndensity..]], fill = .data[["condition"]], color = .data[["condition"]])) +
...
if (dimRedMethodToUse == "pacmap") {
  ...
  if (!is.null(cellsToUmap)) {
    ...
    set.seed(2)
    cellsToPacmap = lapply(levels(md$sample_id), function(x) {
      if (length(which(cdsce[["sample_id"]] == x)) > cellsToUmap) {
        sample(rownames(cdsce)[which(cdsce[["sample_id"]] == x)], cellsToUmap)
      } else {
        rownames(cdsce)[which(cdsce[["sample_id"]] == x)]
      }
    }) %>% unlist
    esPacmap <- esPacmap[as.numeric(cellsToPacmap), ]
    dimred2 <- pacmap_fit(esPacmap, verbose = TRUE, apply_pca = FALSE, n_neighbors = NULL)
    m <- matrix(NA, nrow = ncol(sce), ncol = ncol(dimred2)) %>% data.frame
...
plotUMAPNice <- function(
  ...
  gp1 <- ggplot(df, aes(x = .data[["x"]], .data[["y"]], colour = .data[[colour_by]]))
  if (show_density) {
    gp1 <- gp1 + 
      geom_density_2d(
        data = df[,c("x","y")],
...
mh <- ggplot(gg_df, aes(x = value, y = .data[[..ndensity..]], fill = condition, color = condition)) +
  geom_density(alpha = 0.1) +
