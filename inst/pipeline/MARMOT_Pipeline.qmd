...
if (!is.null(RDataFolder)) {
  ...
  if (file.exists(pq_manifest)) {
    ...
    # Also load flowFrame objects from qs (not decomposed to Parquet)
    ...
    message(glue::glue("🔁 Loading results from .qs files: {RDataFolder}"))
    qs_files <- list.files(RDataFolder, pattern = "\\.(qs)$", full.names = TRUE, recursive = TRUE)
    if (length(qs_files) == 0) stop("No .qs or Parquet files found in the specified RDataFolder.")
...
if (clusteringMethodToUse %in% c("Rphenograph", "FastPG")) {
  ...
  cluster_fn <- function(k) {
    ...
    ids <- factor(ids, levels = 1:nlevels(ids))
...
if (clusteringMethodToUse == "FlowSOM") {
  ...
  for (k in kValuesIWant) {
    sce[[paste0("meta", k)]] <- cluster_ids(sceSOM, paste0("meta", k))
  }

  # Assign selected clustering
  selected_k <- paste0("meta", knn)
  sce$cluster_id <- cluster_ids(sceSOM, selected_k)
...
} else if (exists("qsDir")) {
  file.path(qsDir, "parquet")
} else NULL
...
vars_to_save <- c(
  "QCmini", "cf2", "dimRedMethodToUse", "useMarkers", "framesList", "frames", "nrowsCofactors",
  "sce", "sce2", "sceDS", "coloursList", "md", "downsampleTo", "sceList", "lvlsColours",
  "featuresToCluster", "featuresToUmap", "markerType", "smd", "knn", "clusteringMethodToUse",
  "selectedClustersList", "kids", "numbersOfClusters", "kList", "umapDFList", "kValuesIWant",
  "clustersToMapList", "daList", "dsList", "daPValToUse", "panel", "labs",
  "runScGate"
)

existing_vars <- Filter(exists, vars_to_save)

purrr::walk(existing_vars, function(varname) {
  out_path <- file.path(qsDir, paste0(varname, ".qs"))
  qs::qsave(get(varname, envir = .GlobalEnv), file = out_path, nthreads = nCores, preset = "archive")
  message("✔️ Saved: ", varname)
})

# Also write Parquet format alongside qs (for Shiny app and interoperability)
message("💾 Saving Parquet format...")
MARMOT::save_parquet_data(qsDir, envir = .GlobalEnv)
```
...
if (runScGate && "Gated_Cells" %in% colnames(colData(sce))) {
  ...
  lapply(names(models), function(mpt) {
    ...
    m1 <- models[[mpt]][[1]]
    m2 <- models[[mpt]][[2]]
    if (!m1 %in% rownames(sce)) m1 <- agrep(m1, rownames(sce), value = TRUE)
    if (!m2 %in% rownames(sce)) m2 <- agrep(m2, rownames(sce), value = TRUE)
