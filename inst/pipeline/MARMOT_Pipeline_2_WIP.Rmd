...
### 🔧 File paths
fp       <- "/Users/peterleary/Desktop/FGCZ/MARMOT/Data/Org19"      # Folder with all FCS files
md_fp    <- "/Users/peterleary/Desktop/FGCZ/MARMOT/Data/Org19/My_MARMOT_Analysis_metadata.xlsx"      # Full path to metadata Excel file
...
knn                   <- 40          # Must be in kValuesIWant

### 📉 Dimension Reduction
dimRedMethodToUse <- "UMAP"   # One of: "TSNE", "UMAP", "pacmap"
markersToDimRedBy <- "type"   # One of: "all", "type", "state"
drCellCount       <- NULL     # Cells per sample for DR plots

### 🧼 QC
runQC     <- TRUE
metricsQC <- c("FR", "FS")    # "FR" = flow rate, "FS" = signal acq., "FM" = dynamic range
...
required_vars <- c(
  ...
  "dimRedMethodToUse", "markersToDimRedBy", "drCellCount",
  "runQC", "metricsQC", "useQC", "RDataFolder", "excludeTheseSamples",
...
useMarkers <- smd %>%
  select(matches("include|cluster")) %>%
  unlist(use.names = FALSE) %>%
  na.omit()
names(useMarkers) <- gsub("[-\\s/]", "_", names(useMarkers))
...
framesList <- list("All Cells" = frames)

# Downsample if requested
if (!is.null(downsampleTo)) {
  message(glue::glue("🔻 Downsampling to {downsampleTo} cells per sample..."))
  downsampled <- lapply(frames, function(fcs) {
    if (nrow(fcs) > downsampleTo) {
      set.seed(2)
      idx <- sample(nrow(fcs), downsampleTo)
      fcs[idx, ]
    } else {
      fcs
    }
  })
  framesList <- list("Downsampled" = downsampled, "All Cells" = frames)
}
```
...
if (runQC) {
  ...
  # Collapse multiple QC metrics if needed
  if (length(metricsQC) > 1) {
    metricsQC <- paste(metricsQC, collapse = "_")
    message("📊 Multiple QC metrics selected: ", metricsQC)
  }

  qc_dir <- file.path(fp, "resultsQC")
  ...
  if (!is.null(downsampleTo)) {
    ...
    framesList[["QC"]] <- lapply(framesList[["QC"]], \(x) {
      if (nrow(x) > downsampleTo) {
        set.seed(2)
        x[sample(nrow(x), downsampleTo), ]
      } else {
        x
      }
    })
...
param_df <- pData(parameters(fsList[[1]][[1]])) %>%
  as.data.frame() %>%
...
panel <- param_df %>%
  ...
  filter(!marker_name %in% removeMarkers) %>%
  filter(marker_name %in% useMarkers) %>%
...
# Downsample cells for histogram plotting
framesList[["Marker Histos"]] <- lapply(framesList[["All Cells"]], function(fcs) {
  if (nrow(fcs) > 2000) {
    set.seed(2)
    fcs[sample(nrow(fcs), 2000), ]
  } else {
    fcs
  }
})

# Build data for each marker across samples and cofactors
cofactorDFList <- purrr::map(useMarkers, function(marker) {
  colname <- panel$fcs_colname[panel$marker_name == marker]
  marker_expr <- purrr::map(framesList[["Marker Histos"]], ~ exprs(.x)[, colname])
  df <- tibble(
    sample = rep(names(marker_expr), times = purrr::map_int(marker_expr, length)),
    pretransform = unlist(marker_expr)
  ) %>%
    mutate(
      condition = md$condition[match(sample, md$file_name)],
      sample_id = md$sample_id[match(sample, md$file_name)]
    )

  for (cof in cofactorsToTry) {
    df[[paste0("Cofactor_", cof)]] <- asinh(df$pretransform / cof)
  }

  list(df = df)
}) %>% setNames(useMarkers)

# Generate plots per marker
cofactorPlotList <- future_lapply(useMarkers, function(marker) {
  df <- cofactorDFList[[marker]]$df
  plot_set <- purrr::map(
    c("pretransform", paste0("Cofactor_", cofactorsToTry)),
    function(colname) {
      ggplot(df, aes(x = .data[[colname]], fill = condition, colour = condition)) +
  ...
  plot_set <- setNames(plot_set, c("pretransform", paste0("Cofactor_", cofactorsToTry)))
  plot_set$legend <- get_legend(plot_set[[1]])
  plot_set <- purrr::map(plot_set, ~ .x + theme(legend.position = "none"))
  plot_set
})
names(cofactorPlotList) <- useMarkers
```

```{r prepare_cofactors, results = "hide", eval = is.null(RDataFolder)}
...
if (clusteringMethodToUse %in% c("Rphenograph", "FastPG")) {
  ...
  chosen_k <- paste0("k", knn)
  sce$cluster_id <- sce[[chosen_k]]
  metadata(sce)$cluster_codes <- data.frame(!!clusteringMethodToUse := factor(levels(sce[[chosen_k]])))
  numbersOfClusters <- unique(sce[[chosen_k]])
}
```
...
if (clusteringMethodToUse == "FlowSOM") {
  ...
  numbersOfClusters <- unique(sce[[selected_k]])
}
```
...
if (dimRedMethodToUse == "pacmap") {
  ...
  if (!is.null(cellsToUmap)) {
    cdsce <- colData(sce)
    set.seed(2)
    selected <- unlist(lapply(levels(md$sample_id), function(x) {
      pool <- rownames(cdsce)[cdsce$sample_id == x]
      if (length(pool) > cellsToUmap) sample(pool, cellsToUmap) else pool
    }))
    dimred <- pacmap_fit(es[selected, , drop = FALSE], verbose = TRUE, apply_pca = FALSE)
    out_mat <- matrix(NA, ncol = ncol(dimred), nrow = ncol(sce))
    out_mat[match(selected, colnames(sce)), ] <- dimred
    reducedDim(sce, dimRedMethodToUse) <- out_mat
  } else {
...
# Native DRs
if (dimRedMethodToUse %in% c("UMAP", "TSNE")) {
  sce <- runDR(x = sce, dr = dimRedMethodToUse, cells = cellsToUmap, features = featuresToUmap, assay = exprsToUse)
}
```

```{r prepare_umaps, results = "hide", dependson = c("input settings", "make sce"), eval = is.null(RDataFolder)}

message("🧭 Preparing UMAP dataframe for downstream plotting...")

# Extract reduced dimensions
umap_coords <- reducedDim(sce, dimRedMethodToUse)
...
umapDF <- data.frame(
  ...
  umap_coords,
  check.names = FALSE
) #%>% left_join(cluster_codes(sce), by = c("cluster_id" = clusteringMethodToUse))

umapDF$clusteringMethodToUse <- umapDF$cluster_id

# Add expression matrix (quantile normalised)
qnAssay <- t(assay(sce, "exprsQuantNorm"))
umapDF <- bind_cols(umapDF, as.data.frame(qnAssay))
umapDF <- na.omit(umapDF)

# Rename expression columns using useMarkers mapping
# colnames(umapDF)[colnames(umapDF) %in% useMarkers] <-
#   names(useMarkers)[match(useMarkers, colnames(umapDF))]

umapDFList <- list(All = umapDF)

# Optional downsampling: equal cell count per sample per condition
if (!is.null(drCellPerCondition)) {
  umapDFperSample <- umapDF %>%
    group_by(sample_id) %>%
    group_modify(~ {
...
# Labels with n per condition
source_group <- if (!is.null(drCellPerCondition)) "Downsampled" else "All"
labs <- umapDFList[[source_group]] %>%
  group_by(condition) %>%
  summarise(n = n()) %>%
  mutate(label = paste0(condition, "\n n = ", n)) %>%
  deframe()

names(labs) <- levels(factor(umapDF$condition))
```

```{r cluster_colours, results = "hide", dependson = c("input settings", "generate UMAP df"), eval = is.null(RDataFolder)}
...
for (i in seq_along(contrasts)) {
  cts <- strsplit(contrastsToTest[i], " over ")[[1]]
  daTable <- daList[[i]]

  up1 <- unique(daTable$cluster_id[daTable$logFC >= 0 & daTable[[daPValToUse]] < 0.05])
  up2 <- unique(daTable$cluster_id[daTable$logFC <= 0 & daTable[[daPValToUse]] < 0.05])

  selectedClustersList[[paste0("Contrast ", i, ": ", cts[1], " Up")]] <- as.character(up1)
  selectedClustersList[[paste0("Contrast ", i, ": ", cts[2], " Up")]] <- as.character(up2)
}
