...
#' Check whether a data directory contains MARMOT output
#' @param data_dir Path to the R_files directory
#' @return Invisibly "h5ad"; stops with an informative error if data is absent
detect_data_format <- function(data_dir) {
  h5ad <- file.path(data_dir, "marmot_results.h5ad")
  if (file.exists(h5ad)) return(invisible("h5ad"))
  stop("No MARMOT data found in: ", data_dir,
       ". Please re-run the MARMOT pipeline.")
}
